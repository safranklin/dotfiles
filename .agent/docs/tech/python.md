...
## Python Type Hinting Convention

Use built-in generic types for collections: `list[int]`, `dict[str, int]`, `tuple[str, ...]`, `set[float]` instead of importing from typing module (`List`, `Dict`, etc.).

For unions, use pipe syntax: `str | int` instead of `Union[str, int]`, and `str | None` instead of `Optional[str]`.

Use `@overload` decorator when functions have different return types based on input types or parameter combinations, providing explicit type signatures for each variant before the implementation.

Only import from typing for constructs without built-in equivalents: `TypeVar`, `Protocol`, `Literal`, `overload`, etc. Standard collections and unions use native syntax.

Adopting these principles will prevent the need for unnecessary refactoring when ruff captures these typing sytle quirks as problems.
